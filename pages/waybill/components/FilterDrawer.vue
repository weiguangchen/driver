<template>
	<view class="filter" :class="{ 'active': isFilter }" @click="open">
		<uv-image v-if="!isFilter" src="/static/images/filter/filter.png" width="32rpx" height="32rpx" />
		<uv-image v-else src="/static/images/filter/filter-selected.png" width="32rpx" height="32rpx" />
	</view>
		...
		<view class="filter-wrapper">
			<uv-row customStyle="margin-bottom: 24rpx" gutter="20rpx">
				<uv-col span="12" @click="openType">
					<MySelect :options="typeOptions" v-model="dtType" title="选择类型" :allowEmpty="false"/>
				</uv-col>
			</uv-row>
			...
			<MyDatetime v-if="dateMode === '自定义时间'" v-model="date" @change="changeDate"/>
			<view class="filter-title">装货信息</view>
			<uv-row customStyle="margin-bottom: 24rpx" gutter="20rpx">
				<uv-col span="6">
					<MySelect :options="mfrsOptions" v-model="supplyId" title="选择装货厂家" placeholder="全部装货厂家" @change="changeMfrs"/>
				</uv-col>
				<uv-col span="6">
					<MySelect :options="materialOptions" v-model="materialId" title="选择物料" :disabled="!supplyId" @disabled-click="disabledClick" placeholder="全部物料"/>
				</uv-col>
			</uv-row>
			<view class="filter-title">卸货信息</view>
			<uv-row customStyle="margin-bottom: 24rpx" gutter="20rpx">
				<uv-col span="12">
					<MySelect :options="dischargeOptions" v-model="unloadId" title="选择卸货地" placeholder="全部卸货地" />
				</uv-col>
			</uv-row>
            <view class="filter-title">货主信息</view>
			<uv-row customStyle="margin-bottom: 24rpx" gutter="20rpx">
				<uv-col span="12">
					<MySelect :options="cargoOptions" v-model="ownerId" title="选择货主" placeholder="全部货主公司" :disabled="!supplyId" @disabled-click="disabledClick"/>
				</uv-col>
			</uv-row>
		</view>
...
<script setup>
	...
	import { GetSupplyListByDriver, GetOwnerMaterialsList, GetDriverUnloadPlace, GetOwnerByDriver } from '@/api/index.js';
