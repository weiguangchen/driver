<template>
	<view v-if="showPopup" class="uv-popup" :class="[popupClass, isDesktop ? 'fixforpc-z-index' : '']"
		:style="[{zIndex: zIndex}]">
		<view @touchstart="touchstart">
			<!-- 遮罩层 -->
			<uv-overlay key="1" v-if="maskShow && overlay" :show="showTrans" :duration="duration"
				:custom-style="overlayStyle" :opacity="overlayOpacity" :zIndex="zIndex" @click="onTap"></uv-overlay>
			<uv-transition key="2" :mode="ani" name="content" :custom-style="transitionStyle" :duration="duration"
				...
				<view class="uv-popup__content" :style="[contentStyle]" :class="[popupClass]" @click="clear">
					...
					<uv-image src="/static/images/login/banner.png" width="100%" height="auto" mode="widthFix"
						:duration="0" />
					<view class="footer-wrapper">
						<view class="button-wrapper">
							<uv-button text="手机号快速登录" color="linear-gradient( 270deg, #31CE57 0%, #07B130 100%);" @click="confirm" :loading="loading" :custom-style="{ borderRadius: '16rpx', height: '96rpx', fontWeight: 'bold', fontSize: '30rpx', marginBottom: '24rpx' }"
								open-type="getPhoneNumber" @getphonenumber="getPhoneNumber" :disabled="disabled" />
							<view class="mask" v-if="disabled" @click="maskClick" />
						</view>
						<view class="agreement-box">
							<uv-checkbox-group activeColor="var(--main-color)" shape="circle" v-model="checkboxValue"
								:customStyle="{ flex: 'none' }" size="16px" icon-size="12px" @change="changeCheckbox">
								<uv-checkbox
									:customStyle="{ fontSize: '28rpx', color: 'var(--content-color)' }">请详细阅读并同意</uv-checkbox>
							</uv-checkbox-group>
							<view class="agreement" @click="toAgreement('driverxy')">《用户服务协议》</view>
							<text style="color: var(--content-color);font-size: 14px;">和</text>
							<view class="agreement" @click="toAgreement('driverzc')">《隐私协议》</view>
						</view>
					</view>
					<uv-safe-bottom v-if="safeAreaInsetBottom"></uv-safe-bottom>
					<view v-if="closeable" @tap.stop="close" class="uv-popup__content__close"
						:class="['uv-popup__content__close--' + closeIconPos]"
						hover-class="uv-popup__content__close--hover" hover-stay-time="150">
						<uv-image src="/static/images/login/close.png" width="64rpx" height="64rpx" :duration="0" />
					</view>
				</view>
			</uv-transition>
		</view>
